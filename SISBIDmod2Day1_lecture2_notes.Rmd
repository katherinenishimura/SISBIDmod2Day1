...
```{r}
# change alpha to 0.2 (20%)
# and chage background to white (black and white theme)
# notice that there are some children whose vsae scores increase, and then decrease
# right skewed, therefore consider changing y-axis to log scale
ggplot(autism, aes(x=age2, y=vsae, group=childid)) + 
...
ggplot(autism, aes(x=age2, y=vsae, group=childid, colour=bestest2)) + 
  geom_line(alpha=0.5) + scale_y_log10() + theme_bw()
# bi-modality may be due to these two subgroups
# 
```
...
#geom_smooth(se=F) is for smoothed best-fit line, removing StdErr bars = FALSE
```
